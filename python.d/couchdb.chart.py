...
CHARTS = {
    'activity': {
        'options': [None, 'Overall Activity', 'req/s',
                    'DB activity', 'couchdb.activity', 'stacked'],
        'lines': [
    ...
    'request_methods': {
        'options': [None, 'HTTP request methods', 'req/s',
                    'HTTP traffic breakdown', 'couchdb.request_methods',
                    'stacked'],
        'lines': [
    ...
    'response_codes': {
        'options': [None, 'HTTP response status codes', 'resp/s',
                    'HTTP traffic breakdown', 'couchdb.response_codes',
                    'stacked'],
        'lines': [
    ...
    'open_files': {
        'options': [None, 'Open files', 'files',
                    'Server operations', 'couchdb.open_files', 'line'],
        'lines': [
    ...
    'active_tasks': {
        'options': [None, 'Active task breakdown', 'tasks',
                    'Server operations', 'couchdb.active_tasks', 'stacked'],
        'lines': [
    ...
    'replicator_jobs': {
        'options': [None, 'Replicator job breakdown', 'jobs',
                    'Server operations', 'couchdb.replicator_jobs', 'stacked'],
        'lines': [
    ...
    'erlang_memory': {
        'options': [None, 'Erlang VM memory usage', 'bytes',
                    'Erlang stats', 'couchdb.erlang_vm_memory', 'stacked'],
        'lines': [
    ...
    'erlang_reductions': {
        'options': [None, 'Erlang reductions', 'count',
                    'Erlang stats', 'couchdb.reductions', 'line'],
        'lines': [
    ...
    'erlang_proc_counts': {
        'options': [None, 'Process counts', 'count',
                    'Erlang stats', 'couchdb.proccounts', 'line'],
        'lines': [
    ...
    'erlang_peak_msg_queue': {
        'options': [None, 'Peak message queue size', 'count',
                    'Erlang stats', 'couchdb.peakmsgqueue',
                    'line'],
        'lines': [
    ...
    'db_sizes_file': {
        'options': [None, 'Database sizes (file)', 'KB',
                    'Per-DB stats', 'couchdb.db_sizes_file', 'line'],
        'lines': []
    },
    'db_sizes_external': {
        'options': [None, 'Database sizes (external)', 'KB',
                    'Per-DB stats', 'couchdb.db_sizes_external', 'line'],
        'lines': []
    },
    'db_sizes_active': {
        'options': [None, 'Database sizes (active)', 'KB',
                    'Per-DB stats', 'couchdb.db_sizes_active', 'line'],
        'lines': []
    },
    'db_doc_counts': {
        'options': [None, 'Database # of docs', 'docs',
                    'Per-DB stats', 'couchdb_db_doc_count', 'line'],
        'lines': []
    },
    'db_doc_del_counts': {
        'options': [None, 'Database # of deleted docs', 'docs',
                    'Per-DB stats', 'couchdb_db_doc_del_count', 'line'],
        'lines': []
    }
}
...
class Service(UrlService):
    def __init__(self, configuration=None, name=None):
        ...
        self.node = self.configuration.get('node', 'couchdb@localhost')
    ...
    def check(self):
        ...
        for db in self.dbs:
            dbinfo = self._get_raw_data(self.url + '/' + db)
        ...
        for db in self.dbs:
            ...
            for metric in stats:
                ...
                try:
                    ...
                    continue
                metric_name = 'db_{}_{}'.format(db, '_'.join(metrics_list))
...
def fetch_data(raw_data, metrics):
    ...
    for metric in metrics:
        ...
        if metrics_list[0:2] == ['couchdb', 'httpd_status_codes'] and \
            ...
            metrics_list[2] = '{}xx'.format(int(metrics_list[2]) // 100)
...
def get_peak_msg_queue(data):
    ...
    for queue in iter(queues.values()):
        if type(queue) == dict and 'count' in queue:
            value = queue['count']
        elif type(queue) == int:
            value = queue
        else:
